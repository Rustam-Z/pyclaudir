...
def test_owner_always_allowed_in_dm() -> None:
    for policy in ("owner_only", "allowlist", "open"):
        access = AccessConfig(dm_policy=policy, allowed_users=[], allowed_chats=[])
...
def test_owner_only_blocks_strangers() -> None:
    access = AccessConfig(dm_policy="owner_only")
...
def test_allowlist_permits_listed_user() -> None:
    access = AccessConfig(dm_policy="allowlist", allowed_users=[123])
...
def test_group_check_ignores_dm_policy() -> None:
    """DM policy doesn't affect groups — only allowed_chats matters."""
    access = AccessConfig(dm_policy="open", allowed_chats=[])
    assert not gate(access=access, owner_id=OWNER, chat_id=-100, user_id=999, chat_type="supergroup")
...
def test_save_and_load_round_trip(tmp_path: Path) -> None:
    ...
    config = AccessConfig(dm_policy="allowlist", allowed_users=[1, 2], allowed_chats=[-100])
    save_access(path, config)
    loaded = load_access(path)
    assert loaded.dm_policy == "allowlist"
...
def test_load_returns_defaults_on_missing(tmp_path: Path) -> None:
    ...
    assert loaded.dm_policy == "owner_only"
...
def test_load_handles_invalid_policy(tmp_path: Path) -> None:
    path = tmp_path / "access.json"
    path.write_text('{"dm_policy": "yolo"}', encoding="utf-8")
    loaded = load_access(path)
    assert loaded.dm_policy == "owner_only"
...
def test_atomic_write_survives_read_during_write(tmp_path: Path) -> None:
    ...
    path = tmp_path / "access.json"
    save_access(path, AccessConfig(dm_policy="allowlist", allowed_users=[1]))
...
def test_hot_reload_picks_up_changes(tmp_path: Path) -> None:
    path = tmp_path / "access.json"
    save_access(path, AccessConfig(dm_policy="owner_only"))
    ...
    save_access(path, AccessConfig(dm_policy="allowlist", allowed_users=[99]))
